...
class ArcBall
{
  ...
  void mousePressed()
  {
    v_down = mouse_to_sphere(mouseX*10, mouseY*10);
    q_down.set(q_now);
    q_drag.reset();
  }
  ...
  void mouseDragged()
  {
    v_drag = mouse_to_sphere(mouseX*10, mouseY*10);
  ...
  void initSensorQ()
  {
    v_down = mouse_to_sphere(width / 2, height /2);
    q_down.set(q_now);
    q_drag.reset();
  }
  ...
  void sensorQ()
  {
    v_drag = mouse_to_sphere(sensorVectX, sensorVectY);
    q_drag.set(Vec3.dot(v_down, v_drag), Vec3.cross(v_down, v_drag));
  }
